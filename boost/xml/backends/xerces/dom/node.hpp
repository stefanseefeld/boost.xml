#ifndef boost_xml_dom_backends_xerces_node_hpp_
#define boost_xml_dom_backends_xerces_node_hpp_

#include <boost/xml/dom/nodefwd.hpp>
#include <boost/xml/backends/xerces/dom/iterator.hpp>
#include <stdexcept>
...
template <typename S>
class node : public detail::wrapper<detail::DOMNode*>
{
  ...
  friend node_ptr<node<S> > detail::ptr_factory<node<S> >(detail::DOMNode *);
  friend node_ptr<node<S> const> detail::ptr_factory<node<S> const>(detail::DOMNode *);
...
public:
  ...
  S base() const { return converter<S>::out(this->impl()->getBaseURI());}
...
protected:
  node(detail::DOMNode *n) : detail::wrapper<detail::DOMNode*>(n) {}
  node(node<S> const &n) : detail::wrapper<detail::DOMNode*>(n) {}
  node<S> &operator=(node<S> const &n) 
  {
    detail::wrapper<detail::DOMNode*>::operator=(n);
    return *this;
  }
private:
